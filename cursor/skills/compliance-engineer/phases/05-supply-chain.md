...
Audit every dependency in the project for known vulnerabilities, license risk, maintenance health, and supply chain integrity. Generate a Software Bill of Materials and evaluate the full dependency tree -- not just direct dependencies. compliance-engineer is the SOLE AUTHORITY on application-layer dependency security analysis. DevOps handles container image CVE scanning at the infrastructure layer; this phase covers library and package vulnerabilities. Generate all outputs in `.sdlc-automation-agent/compliance-engineer/supply-chain/`.

## Context Bridge

Read Phase 4 outputs from `.sdlc-automation-agent/compliance-engineer/data-security/`. Data security findings may reveal dependencies that handle encryption or PII processing -- these deserve elevated scrutiny in the supply chain audit. Also reference Phase 2 code audit A06 (Vulnerable and Outdated Components) for any dependency flags already raised.

## Inputs

- Phase 2 code audit -- `.sdlc-automation-agent/compliance-engineer/code-audit/` (A06 findings)
- Phase 4 data security -- `.sdlc-automation-agent/compliance-engineer/data-security/`
...
Run the appropriate license scanner for each detected ecosystem. Do not rely on manual inspection alone — scanners enumerate transitive dependencies that manual review misses:

```bash 
# Node.js 
npx license-checker --production --json \
  --out .sdlc-automation-agent/compliance-engineer/supply-chain/license-inventory.json

# Python
pip-licenses --format=json \  
  --output-file .sdlc-automation-agent/compliance-engineer/supply-chain/license-inventory.json  

# Go  
go-licenses report ./... 2>&1 \
  > .sdlc-automation-agent/compliance-engineer/supply-chain/license-inventory.txt

# If no tool available: extract from package.json / requirements.txt manually
```

Parse the output and apply the risk classification table above. Write `license-compliance.md` with:
```markdown
## License Compliance Report

- **Total dependencies scanned:** N
- **Risk distribution:** None: X | Low: Y | Medium: Z | High: A | Unknown: B

### Flagged Dependencies (High or Unknown Risk)

| Package | Version | License | Risk | Recommended Action |
|---------|---------|---------|------|--------------------| 

### Compliance Verdict 

PASS — no High or Unknown licensed production dependencies
REVIEW REQUIRED — High-risk licenses present (see table above)
BLOCK — Unknown licenses in production dependency path (legal review required) 
```

If verdict is BLOCK: this is a **High** severity finding (same gate as CRITICAL CVE).
If verdict is REVIEW REQUIRED: **Medium** severity finding.
...
Write all outputs to `.sdlc-automation-agent/compliance-engineer/supply-chain/`:
