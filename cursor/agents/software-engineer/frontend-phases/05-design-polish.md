...
## 5.0 Design Handoff Short-Circuit (conditional)

**IF the current story has `design_ref` AND the handoff's `approved_at` is set**, Phase 5 does not run style selection or domain research — the client-approved prototype IS the style directive, and re-running WebSearch-driven trend research at this point can only *diverge* from what the client already approved.

```
design_ref=$(${TRACKER_CLI} get-story "${STORY_ID}" --field design_ref 2>/dev/null)

IF [ -n "$design_ref" ] AND [ -f "$design_ref" ]: 
  approved_at=$(grep '^approved_at:' "$design_ref" | awk '{print $2}')
  IF [ -n "$approved_at" ]: 
    # Client-approved prototype exists — skip discovery, use it as truth
    - SKIP the Style Selection block below (no AskUserQuestion, no auto-select)
    - SKIP 5.1 Domain Research (WebSearch) entirely 
    - SKIP 5.2 and 5.3 colour/typography selection (handoff already decided)
    - LOAD:
        * Design Tokens section → style directive for colour, typography, spacing
        * Layout Intent section → grid/responsive/breakpoint directive
        * Interaction Spec section → hover/focus/motion directive
    - PROCEED directly to 5.4 (Visual Hierarchy Polish) treating handoff as truth
    - RECORD in SE receipt:
        design_polish_source: "handoff"   # not "web-research"
        design_polish_ref:    "$design_ref" 
    - RETURN at Phase 5 completion

ELSE (no design_ref, or handoff not yet approved):
  Follow the Style Selection and research flow below. 
``` 

Rationale: when a client approved a specific prototype, the prototype's colour palette, typography, and interaction patterns are the definition of "domain-appropriate". WebSearch-driven trend research at this point is at best redundant and at worst contradicts the approved direction — triggering avoidable rework at the next Sprint Review.

---

## Style Selection — Engagement Mode Aware

Read engagement mode from `.sdlc-automation-agent/.orchestrator/settings.md`.  
...
Report the selection: `Style: {selected} (auto-selected for {domain} — Autonomous mode)`
...
Document findings in `.sdlc-automation-agent/software-engineer-frontend/docs/design-research.md` including the selected style and how it maps to the domain.
...
- `.sdlc-automation-agent/software-engineer-frontend/docs/design-research.md` — Domain research, competitive analysis, trend findings 
- `.sdlc-automation-agent/software-engineer-frontend/docs/design-decisions.md` — Color choices with rationale, typography selection, visual hierarchy rules
...
**Design review (mode-aware):** Autonomous — proceed to Phase 6, report design summary. Controlled — present full design system comparison via AskUserQuestion.

## Quality Bar

"Default blue with system fonts" is NOT sdlc-automation-agent. 
...
THAT is sdlc-automation-agent — intentional design that matches the user's chosen style.
