...
```bash
# From this repo root — SDLC 13-agent orchestrator
claude --plugin-dir /path/to/agents

# Or use the portable package (copy .claude into a product repo first)
./scripts/package-claude.sh
claude --plugin-dir /path/to/product/.claude
```
...
| `stack-vue` | Vue 3 SPA (Composition API, Pinia, Router) |
| `stack-nuxt` | Nuxt 4+, Nuxt UI/Content/Hub, VueUse |
| `stack-spring` | Spring Boot / Java |
| `stack-golang` | Go |
| `stack-aws` / `stack-azure` | Cloud |

```bash
claude --plugin-dir plugins/stack-vue --plugin-dir plugins/stack-nuxt
```
...
|--------|--------|--------|----------|
| **sdlc-automation-agent** (repo root) | Orchestrator + 13 agents | This repo | Full delivery pipeline |
| **stack-golang** | 43 | `cc-skills-golang` | Go services, CLI, gRPC |
| **stack-spring** | 19 | `spring-boot-skills` | Java / Spring Boot 3.x backend |
...
| **Java + AWS** | root + `system-design` + `stack-spring` + `stack-aws` + `sdlc-workflows` |
| **Java + Azure** | root + `system-design` + `stack-spring` + `stack-azure` + `sdlc-workflows` |
| **Next.js + Java + AWS** | root + `system-design` + `stack-spring` + `stack-frontend` + `stack-aws` + `sdlc-workflows` |
...
| **Software Engineer** | stack-golang / stack-spring / stack-frontend, sdlc-workflows | next-best-practices, rest-api-conventions, golang-project-layout, incremental-implementation |
