...
|-------|------|----------------------------|
| **SDLC delivery agents (13 roles)** | `agents/{role}/` | Yes — via root `claude-agents/*.md` stubs (file copies) |
...
After `./scripts/sync-from-new-skills.sh` (or full `./scripts/sync-all.sh`):
