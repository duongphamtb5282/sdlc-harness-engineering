...
1. `.sdlc-automation-agent/research-advisor/handoff/context-package.md` — may contain scale, constraints, decisions 
2. `docs/requirements/BRD.md` — business context, NFRs, scope, constraints, data model (or `paths.brd` from `.sdlc-automation-agent.yaml`)
...
4. `.sdlc-automation-agent/.orchestrator/codebase-context.md` — brownfield context
...
- Tech mentions in BRD or research advisor context -> use those, else conservative defaults
- Default: modular monolith, managed services, single region, single DB
- Log: `✓ Autonomous mode — auto-deriving architecture from BRD`
