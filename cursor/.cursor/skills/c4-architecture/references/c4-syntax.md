...
C4Deployment
  ...
  Rel(static, lb, "Forwards requests", "HTTPS")
...
`Rel()` must connect **containers** (`Container`, `ContainerDb`, etc.), not `Deployment_Node` aliases. Cross-zone traffic should use the inner container IDs — e.g. `Rel(static, lb, ...)` not `Rel(cdn, alb, ...)`.
