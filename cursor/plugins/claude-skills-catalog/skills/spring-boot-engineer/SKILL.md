...
## Stack plugin (preferred for implementation)

When `plugins/stack-spring` is installed, load topic skills from `${CLAUDE_PLUGIN_ROOT}/plugins/stack-spring/skills/` **before** generic examples in this file. Precedence: [STACK-RULES.md](../../stack-spring/STACK-RULES.md).

| Task | stack-spring skill |
|------|-------------------|
| REST controllers / DTOs | `rest-api-conventions` |
| Exception handling | `problem-details-rfc9457` |
| Layer boundaries | `layered-architecture` or `hexagonal-architecture` (detect from packages) |
| OpenAPI spec-first | `openapi-first` |
| JPA / Flyway | `spring-data-jpa`, `flyway-migrations` |
| Auth (custom JWT) | `spring-security-jwt` |
| Auth (external IdP) | `oauth2-resource-server` |
| Tests | `testing-pyramid` |
| DDD aggregates | `domain-driven-design` |
| Java PR review | `java-code-review`, `jpa-patterns`, `api-contract-review` |
| JPA debugging | `jpa-patterns` (with `spring-data-jpa`) |
| Security audit (read-only) | `security-audit` |

Tech pack verify/layout: `packs/languages/java-spring/`. Merge reviews: `docs/spring-boot-skills-merge-review.md`, `docs/claude-code-java-merge-review.md`.
