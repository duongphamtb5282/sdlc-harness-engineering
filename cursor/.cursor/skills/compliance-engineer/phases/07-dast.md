...
nuclei -u {BASE_URL} \ 
  ...
  -j -o .sdlc-automation-agent/compliance-engineer/dast/nuclei-raw.json \ 
  2>/dev/null 
``` 
...
Write all outputs to `.sdlc-automation-agent/compliance-engineer/dast/`:
...
Also append DAST findings to `.sdlc-automation-agent/compliance-engineer/issues.json` (same schema as Phase 2 static findings) with `"source": "dast"` field.
...
```json
{
  ...
  "backend": "claude",
  "model": "",
  "artifacts": [".sdlc-automation-agent/compliance-engineer/dast/findings.md"],
  "metrics": {
  ...
  "verification_commands": [
    {
      "command": "test -f .sdlc-automation-agent/compliance-engineer/dast/findings.md && echo 'DAST findings file exists'",
      "exit_code": 0,  
