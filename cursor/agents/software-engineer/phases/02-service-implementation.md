...
## Phase 2a: Shared Foundations Gate

**This phase file is loaded for both Phase 2a (Shared Foundations) and Phase 2b (per-service implementation). Read the orchestrator prompt to determine which sub-phase is active.**

**If you are executing Phase 2a (Shared Foundations):**  

Write the following to `libs/shared/` before returning:
...
After ALL shared foundation files are written, write the foundations checkpoint:

```json  
{
  "completed_at": "<ISO-8601>",  
  "artifacts": ["libs/shared/types/index.ts", "libs/shared/errors/...", "..."]
}
```

Write to `.sdlc-automation-agent/software-engineer/foundations-complete.json`.

**MUST write this file before proceeding. The orchestrator checks for it before spawning service agents.**

--- 

## Objective

Implement each service identified in Phase 1, one at a time, in dependency order (shared libs first, then core services, then dependent services). Follow clean architecture with thin handlers, pure service logic, and data-access repositories.

### TDD Enforcement (Iron Law 4)

**Every new endpoint, business rule, and utility function follows Red-Green-Refactor:** 

1. **RED** — Write a test describing the expected behavior. Run it. It MUST fail. If it passes, your test is wrong. 
2. **GREEN** — Write the MINIMUM code to make the test pass. No "while I'm here" additions.
3. **REFACTOR** — Clean up both test and implementation. Run tests again to confirm they still pass.

**Per-endpoint workflow:**
``` 
1. Write handler test (expected request → expected response)     → Run → FAIL
2. Write handler skeleton (just enough to compile)                → Run → FAIL (wrong response) 
3. Write service test (expected input → expected output)          → Run → FAIL
4. Write service logic                                            → Run → PASS
5. Wire handler to service                                        → Run handler test → PASS
6. Write repository test (expected query → expected result)       → Run → FAIL
7. Write repository implementation                                → Run → PASS
8. Wire service to repository                                     → Run all tests → PASS
9. Refactor — clean up, extract helpers, improve names            → Run all tests → PASS
``` 

**Exceptions (test-after is acceptable for):** Generated code (OpenAPI codegen, protobuf stubs), configuration files (docker-compose, CI/CD), pure boilerplate (DI wiring, app bootstrap), migration files. 

**Verification:** After implementing each service, run the full test suite and report results. Do NOT proceed to the next service if tests are failing. 
...
**Code review (mode-aware):** Autonomous — proceed immediately, report metrics. Controlled — present code for detailed review via AskUserQuestion.
...
| Migration UP/DOWN | After writing any migration file: run UP on test DB, verify schema, run DOWN, verify rollback |

## 2.9 — Migration Verification (MANDATORY when migrations are written)

After writing any migration file (`schemas/migrations/` or `**/migrations/`), verify it before committing:

```bash
# 1. Run UP — must exit 0
{migration_up_command}   # e.g. npx prisma migrate deploy / alembic upgrade head / goose up

# 2. Capture schema snapshot
{schema_inspect_command}  # e.g. npx prisma db pull --print / pg_dump --schema-only

# 3. Run DOWN — must exit 0
{migration_down_command}  # e.g. npx prisma migrate reset --skip-seed / alembic downgrade base / goose down

# 4. Re-run UP to leave DB in correct state 
{migration_up_command}
```

Add migration test results to the SE receipt under:
```json
"migration_tests": {
  "migration_files": ["schemas/migrations/001_create_users.sql"],
  "up": "pass",
  "down": "pass",
  "data_preserved": true
}
...
If DOWN is not implemented (intentionally irreversible migration): log in receipt as `"down": "not_implemented"` with justification. A missing DOWN without justification is a **High** severity finding — no rollback path means no clean recovery from a bad deploy.
