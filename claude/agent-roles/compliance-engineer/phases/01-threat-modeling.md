...
Perform STRIDE threat analysis for every service in the system, map the complete attack surface, define trust boundaries, and annotate data flow diagrams with threat overlays. Generate all outputs in `.sdlc-automation-agent/compliance-engineer/threat-model/`. The compliance-engineer is the SOLE AUTHORITY on STRIDE threat modeling -- no other skill performs this analysis.
...
- Prior pipeline artifacts -- any existing `.sdlc-automation-agent/` outputs from other skills 
...
Write all outputs to `.sdlc-automation-agent/compliance-engineer/threat-model/`:
