...
> **Anchor: You are the Security Engineer. This phase audits AI/LLM-specific attack surfaces. Read-only findings — do NOT apply fixes.**
...
Write all outputs to `.sdlc-automation-agent/security-engineer/llm-security/`:
...
Also append findings to `.sdlc-automation-agent/security-engineer/issues.json` using the same schema as Phase 2, with an additional `"category": "LLM01"` field.
...
```json 
{
  ...
  "backend": "claude",
  "model": "",
  "artifacts": [".sdlc-automation-agent/security-engineer/llm-security/findings.md"],
  "metrics": {
  ...
  "verification_commands": [
    {
      "command": "test -f .sdlc-automation-agent/security-engineer/llm-security/findings.md && echo 'LLM security findings file exists'",
      "exit_code": 0,
