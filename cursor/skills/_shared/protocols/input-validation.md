...
cat .sdlc-automation-agent.yaml 2>/dev/null
```

If `.sdlc-automation-agent.yaml` exists, use its `paths.*` values for all file lookups. If not, use the default paths documented in each skill.
...
Glob("schemas/**/*")
Glob("services/**/*")
Read(".sdlc-automation-agent.yaml")
``` 
