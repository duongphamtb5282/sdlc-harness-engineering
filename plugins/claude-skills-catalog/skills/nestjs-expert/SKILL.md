---
name: nestjs-expert
description: Creates and configures NestJS modules, controllers, services, DTOs, guards, and interceptors for enterprise-grade TypeScript backend applications. Use when building NestJS REST APIs or GraphQL services, implementing dependency injection, scaffolding modular architecture, adding JWT/Passport authentication, integrating TypeORM or Prisma, Kafka microservices (@MessagePattern/@EventPattern), BullMQ queues, health checks, or working with .module.ts, .controller.ts, and .service.ts files. Invoke for guards, interceptors, pipes, validation, Swagger documentation, unit/E2E testing, and NestJS best-practice reviews (40 rules: arch, di, security, perf, db, micro, devops).
license: MIT
metadata:
...
| **Best practices index** | `references/rules-index.md` | 40 rules by category (arch, di, security, micro, …) |
| Architecture rules | `references/rules/arch-*.md` | Modules, repository pattern, events, circular deps |
| DI rules | `references/rules/di-*.md` | Constructor injection, scopes, tokens |
| Security rules | `references/rules/security-*.md` | JWT, guards, validation, rate limiting |
| Microservices rules | `references/rules/micro-*.md` | MessagePattern vs EventPattern, queues, health |
| Database rules | `references/rules/db-*.md` | Transactions, migrations, N+1 |
| Performance rules | `references/rules/perf-*.md` | Caching, async hooks, query optimization |
| DevOps rules | `references/rules/devops-*.md` | ConfigModule, logging, graceful shutdown |

## Best practices (from agent-nestjs-skills)

When reviewing or implementing NestJS code, load rules from `references/rules/` by prefix:

| Priority | Category | Prefix | Load when |
|----------|----------|--------|-----------|
| 1 | Architecture | `arch-` | Module layout, god services, circular deps |
| 2 | Dependency injection | `di-` | Providers, scopes, testability |
| 3 | Error handling | `error-` | Exception filters, HTTP exceptions |
| 4 | Security | `security-` | Auth, validation, guards, rate limits |
| 5 | Performance | `perf-` | Caching, DB queries, lazy loading |
| 6 | Testing | `test-` | TestingModule, mocks, Supertest E2E |
| 7 | Database | `db-` | Transactions, migrations, N+1 |
| 8 | API design | `api-` | Pipes, interceptors, versioning, DTOs |
| 9 | Microservices | `micro-` | Kafka RPC, queues (BullMQ), health checks |
| 10 | DevOps | `devops-` | Config, logging, graceful shutdown |

**Seat-reservation / Kafka RPC:** always load `micro-use-patterns.md` and `micro-use-queues.md` when adding `@MessagePattern` handlers or background jobs.

Full index: `references/rules-index.md` (40 rules with incorrect vs correct examples).
